...
export default function ChatPanel({ app }: { app: LoanApplication }) {
  ...
  return (
    <Card className="flex h-[420px] flex-col overflow-hidden">
      <div className="border-b border-slate-200 px-4 py-3 font-semibold dark:border-slate-700">AI Loan Officer Assistant</div>
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.map((m, i) => (
          <div key={i} className={`max-w-[90%] rounded-2xl px-3 py-2 text-sm ${m.role === 'user' ? 'ml-auto bg-blue-700 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
            {m.text}
          </div>
        ))}
        {loading && <div className="inline-flex items-center gap-2 text-xs text-slate-500"><Spinner /> Thinking…</div>}
      </div>
      <div className="flex flex-wrap gap-2 border-t border-slate-200 p-3 dark:border-slate-700">
        {suggestions.map((s) => (
          <button key={s} onClick={() => send(s)} className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium hover:bg-blue-50 dark:bg-slate-800">
            {s}
          </button>
        ))}
      </div>
